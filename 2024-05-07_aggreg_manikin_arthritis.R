...
# read in data ------
data1 <- read.csv(here("datasets_arthritis", "2024-05-05_data_arthritis.csv"))
...
# creating column with number of minutes sitting per week
cleanlightmeansdata1 <- cleanlightmeansdata1 %>%
  mutate(sit = (sedentary_d*sedentary_m))
hist(cleanlightmeansdata1$sit)
...
data2$sit_c <- scale (data2$sit, center = TRUE, scale = TRUE)
