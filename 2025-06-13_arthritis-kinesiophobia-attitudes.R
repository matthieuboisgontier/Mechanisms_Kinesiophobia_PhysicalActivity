...
cleanlightdata1$kp3r[is.element(cleanlightdata1$kp3 ,c("0"))] <- "0"
...
cleanlightdata1$kp4r[is.element(cleanlightdata1$kp4 ,c("0"))] <- "0"
...
cleanlightdata1$kp7r[is.element(cleanlightdata1$kp7 ,c("0"))] <- "0"
...
cleanlightdata1$kp11r[is.element(cleanlightdata1$kp11 ,c("0"))] <- "0"
...
DataAggreg = aggregate (cbind(c8, c16, kpsum, attitude, intention, pain, age, sex01, mvpa, gender01,
...
data_non_OA <- DataAggreg %>% filter(!(c8 %in% "2" | c16 %in% "2"))
unique(data_non_OA$id) 
nrow(data_non_OA)
describe (data_non_OA$age)
describe (data_non_OA$mvpa)
describe (data_non_OA$kpsum)
describe (data_non_OA$pain)
describe (data_non_OA$bmi)
describe (data_non_OA$sum_chronic)
sum(with(data_non_OA,sex01 == "1"))
sum(with(data_non_OA,sex01 == "2"))
...
## Testing statistical differences between OA and non OA
data_non_OA$group <- "No_OA"
data_OA$group <- "OA"

data_combined <- rbind(
  data_non_OA[, c("group", "age", "mvpa", "kpsum", "pain", "bmi", "sum_chronic")],
  data_OA[, c("group", "age", "mvpa", "kpsum", "pain", "bmi", "sum_chronic")]
)

data_combined$group <- factor(data_combined$group, levels = c("No_OA", "OA"))

# Variables to test
vars_to_test <- c("age", "mvpa", "kpsum", "pain", "bmi", "sum_chronic")
...
for (var in vars_to_test) {
  ...
  # Run t-test
  ttest <- t.test(formula, data = data_combined)
  p_val <- ttest$p.value
  
  # Run Cohen's d
  d_val <- cohen.d(formula, data = data_combined)$estimate
...
results$P_value <- format.pval(results$P_value, digits = 6, eps = .Machine$double.eps)
...
# Mediation analysis of the association between kinesiophobia and intention by attitude
mediation.1 <- lm (intention ~ kpsum_c + pain_c  + biasapcorr_c + biassedcorr_c + age_c + sex01 + bmi_c + sum_chronic, data=DataAggreg, subset = (c8 == 2) & (c16 == 1), na.action=na.omit) 
summary(mediation.1)
confint(mediation.1)
...
mediation.3 <- lm (intention ~ attitude_c + pain_c  + biasapcorr_c + biassedcorr_c  + age_c + sex01 + bmi_c + sum_chronic, data=DataAggreg, subset = (c8 == 2) & (c16 == 1), na.action=na.omit) 
summary(mediation.3)
confint(mediation.3)
plot(allEffects(mediation.3), select = 1)

mediation.4 <- lm (intention ~ kpsum_c + attitude_c + pain_c  + biasapcorr_c + biassedcorr_c  + age_c + sex01 + bmi_c + sum_chronic, data=DataAggreg, subset = (c8 == 2) & (c16 == 1), na.action=na.omit) 
summary(mediation.4)
confint(mediation.4)
...
mediation_model.5 <- ' # direct effect
             intention ~ p*kpsum_c + c1*pain_c + c2*biasapcorr_c + c3*biassedcorr_c+ c4*age_c + c5*sex01 + c6*bmi_c + c7*sum_chronic
           # mediator
             attitude ~ a*kpsum_c + c8*pain_c + c9*biasapcorr_c + c10*biassedcorr_c+ c11*age_c + c12*sex01 + c13*bmi_c + c14*sum_chronic
             intention ~ b*attitude 
           # indirect effect (a*b)
             ab := a*b 
           # total effect
             total := p + (a*b)
         '
fit <- sem(mediation_model.5, data = data3)
summary(fit)
...
mediation.6 <- lm (mvpa ~ kpsum_c + pain_c  + biasapcorr_c + biassedcorr_c + age_c + sex01 + bmi_c + sum_chronic, data=DataAggreg, subset = (c8 == 2) & (c16 == 1), na.action=na.omit) 
summary(mediation.6)
confint(mediation.6)
plot(allEffects(mediation.6), select = 1)

mediation.7 <- lm (intention ~ kpsum_c + pain_c + biasapcorr_c + biassedcorr_c + age_c + sex01 + bmi_c + sum_chronic, data=DataAggreg, subset = (c8 == 2) & (c16 == 1), na.action=na.omit) 
summary(mediation.7)
confint(mediation.7)
plot(allEffects(mediation.7), select = 1)

mediation.8 <- lm (mvpa ~ intention_c + pain_c  + biasapcorr_c + biassedcorr_c  + age_c + sex01 + bmi_c + sum_chronic, data=DataAggreg, subset = (c8 == 2) & (c16 == 1), na.action=na.omit) 
summary(mediation.8)
confint(mediation.8)
plot(allEffects(mediation.8), select = 1)

mediation.9 <- lm (mvpa ~ kpsum_c + intention_c + pain_c  + biasapcorr_c + biassedcorr_c  + age_c + sex01 + bmi_c + sum_chronic, data=DataAggreg, subset = (c8 == 2) & (c16 == 1), na.action=na.omit) 
summary(mediation.9)
confint(mediation.9)
plot(allEffects(mediation.9), select = 1)
...
mediation_model.10 <- ' # direct effect
             mvpa_c ~ p*kpsum_c + c1*pain_c + c2*biasapcorr_c + c3*biassedcorr_c+ c4*age_c + c5*sex01 + c6*bmi_c + c7*sum_chronic
           # mediator
             intention ~ a*kpsum_c + c8*pain_c + c9*biasapcorr_c + c10*biassedcorr_c+ c11*age_c + c12*sex01 + c13*bmi_c + c14*sum_chronic
             mvpa_c ~ b*intention
           # indirect effect (a*b)
             ab := a*b 
           # total effect
             total := p + (a*b)
         '
fit <- sem(mediation_model.10, data = data3)
summary(fit)
...
lmm1.1 <- lmer(relativelatencygeomdirection  ~ 1 + approach*stimulus_ap0_sed1*kpsum + attitude_c + pain_c + mvpa_c + age_c + sex01 + bmi_c + computer01 + sum_chronic + (1|id) + (1|pictograms), 
...
plot(allEffects(lmm1.1, xlevels = list(kpsum = c(0:77))), select = 9)
