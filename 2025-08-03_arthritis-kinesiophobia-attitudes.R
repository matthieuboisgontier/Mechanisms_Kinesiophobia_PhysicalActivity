...
cor.test(cleandata1$attitude1, cleandata1$attitude2, method = "pearson")
...
data_OA <- DataAggreg %>% filter((c8 %in% "2") & (c16 %in% "1")) # participants with Osteoarthritis and no arthritis
unique(data_OA$id) 
...
data_RA <- DataAggreg %>% filter(c16 %in% "2") # participants with RA
unique(data_RA$id) 
...
data_nonOA <- DataAggreg %>% filter((c8 %in% "1") & (c16 %in% "1")) # participants without OA and without RA
unique(data_nonOA$id) 
describe (data_nonOA$age)
describe (data_nonOA$mvpa)
describe (data_nonOA$kpsum)
describe (data_nonOA$pain)
describe (data_nonOA$bmi)
describe (data_nonOA$sum_chronic)
sum(with(data_nonOA,sex01 == "1"))
sum(with(data_nonOA,sex01 == "2"))

data_OA_RA <- DataAggreg %>% filter((c8 %in% "2") & (c16 %in% "2")) # participants with OA and RA
unique(data_OA_RA$id) 
describe (data_OA_RA$age)

data_noOA_RA <- DataAggreg %>% filter((c8 %in% "1") & (c16 %in% "2")) # participants with OA and RA
unique(data_noOA_RA$id) 
describe (data_noOA_RA$age)

### STATISTICAL MODELS

#Effect of kinesiophobia on MVPA when both OA and RA are included
lm1.1 <- lm (mvpa ~ kpsum_c*c8.1.0 + pain_c  + age_c + sex01 + bmi_c + sum_chronic, data=DataAggreg, na.action=na.omit) # arthritis
summary(lm1.1)
confint(lm1.1) #95% confidence interval

#Effect of kinesiophobia on MVPA in OA vs no OA
...
plot(allEffects(lm1.2), select = 6, ylim = c(0, 1400))
...
moderation.1 <- lm (mvpa ~ kpsum_c*biasapcorr_c + kpsum_c*attitude_c + pain_c +  age_c + sex01 + bmi_c + sum_chronic, data=DataAggreg, subset = (c8 == 2) & (c16 == 1), na.action=na.omit) 
summary(moderation.1)
plot(allEffects(moderation.1))
#plot(allEffects(moderation.1))

# Figure 4
moderation.fig <- lm (mvpa ~ kpsum_c*biasapcorr + kpsum_c*attitude_c + pain_c +  age_c + sex01 + bmi_c + sum_chronic, data=DataAggreg, subset = (c8 == 2) & (c16 == 1), na.action=na.omit) 
summary(moderation.fig)
plot(allEffects(moderation.fig), select = 6)
# estimates and p values
DataAggreg$biasapcorr_centered_on_500 <- DataAggreg$biasapcorr - 500
moderation.fig <- lm (mvpa ~ kpsum_c*biasapcorr_centered_on_500 + kpsum_c*attitude_c + pain_c +  age_c + sex01 + bmi_c + sum_chronic, data=DataAggreg, subset = (c8 == 2) & (c16 == 1), na.action=na.omit) 
summary(moderation.fig)

## Mediation analysis by attitude in Osteo Arthritis 
mediation.1 <- lm (mvpa ~ kpsum_c + pain_c  + biasapcorr_c +  age_c + sex01 + bmi_c + sum_chronic, data=DataAggreg, subset = (c8 == 2) & (c16 == 1), na.action=na.omit) 
summary(mediation.1)
#confint(mediation.1)
plot(allEffects(mediation.1 ), select = 1)

mediation.2A <- lm (biasapcorr ~ kpsum_c + pain_c + age_c + sex01 + bmi_c + sum_chronic, data=DataAggreg, subset = (c8 == 2) & (c16 == 1), na.action=na.omit) 
summary(mediation.2A)
#confint(mediation.2A)
plot(allEffects(mediation.2A), select = 1)

mediation.2B <- lm (attitude ~ kpsum_c + pain_c + age_c + sex01 + bmi_c + sum_chronic, data=DataAggreg, subset = (c8 == 2) & (c16 == 1), na.action=na.omit) 
summary(mediation.2B)
#confint(mediation.2B)
plot(allEffects(mediation.2B), select = 1)

mediation.3 <- lm (mvpa ~ attitude_c + kpsum_c + pain_c  + biasapcorr_c + age_c + sex01 + bmi_c + sum_chronic, data=DataAggreg, subset = (c8 == 2) & (c16 == 1), na.action=na.omit) 
summary(mediation.3)
#confint(mediation.3)
